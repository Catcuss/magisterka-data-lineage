...
niniejszej pracy jest zastosowanie i ewaluacja skutecznośći algorytmów predykcji brakujących krawędzi
...
grafu zależności w~warstwie ETL uniemożliwia przeprowadzenie takiego audytu~\cite{Boinski2025}.
...
\begin{itemize}
  \item \textbf{Scenariusz~A} — losowe usunięcie 20\% krawędzi ze~zbioru treningowego,
  \item \textbf{Scenariusz~B} — symulacja ukrycia zależności przez tabelę pośrednią (staging),
...
\begin{itemize}
  ...
  \item \textbf{AUC-PR} — pole pod krzywą precyzja–czułość; szczególnie
        miarodajne przy silnym niezbalansowaniu klas, gdy klasa pozytywna
        (brakujące krawędzie) stanowi ułamek wszystkich par węzłów.
\end{itemize}
...
lineage links \cite{Boinski2025}}
...
\textbf{Zaproponowane rozwiązanie}
...
ograniczeniem jest zależność jakości predykcji od~spójności konwencji
...
\subsection{An open dataset of data lineage graphs \cite{Chen2024}}
...
Heurystyki grafowe stanowią najprostszą klasę metod predykcji krawędzi
...
połączenia~\cite{Barabasi1999}; jako heurystyka predykcji krawędzi PA
...
wysoki stopień węzła \textit{table} wskazuje na tabelę centralną — często
pośrednią warstwę integracyjną w~potoku ETL. Para $(u, v)$, w~której oba
...
Adamic-Adar) wymagają adaptacji do struktury bipartite. PA nie wymaga takiej
...
predykcji w~porównaniu z~miarami uwzględniającymi topologię sąsiedztwa.
