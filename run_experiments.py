...
Użycie:
    python run_experiments.py              # DLG1–DLG4 (małe, szybkie)
    python run_experiments.py --all        # wszystkie 18 grafów
...
Opcje:
    ...
    --no-ml        pomiń modele ML (RF, RUSBoost) — szybsze
...
def run_one(dlg_id: int, seed: int, run_ml: bool) -> list[dict]:
    ...
    for method in HEURISTICS:
        ...
        # Dla heurystyk szukamy optymalnego progu
        if len(np.unique(scores)) > 1:
            t = best_threshold_f1(y_true, scores)
        else:
            t = 0.5
        m = evaluate_split(splits, scores, threshold=t)
        results.append({
            "graph": label, "nodes": n_nodes,
            "train_pos": n_train, "test_pos": n_test,
            "algorithm": method, **m,
        })
    ...
    for model_name, ModelClass in ML_MODELS.items():
        try:
            ...
            results.append({
                "graph": label, "nodes": n_nodes,
                "train_pos": n_train, "test_pos": n_test,
                "algorithm": model_name, **m,
            })
        except Exception as e:
            print(f"  [{label}] {model_name} błąd: {e}")
...
def main():
    ...
    for dlg_id in dlg_ids:
        ...
        results = run_one(dlg_id, seed=args.seed, run_ml=run_ml)
