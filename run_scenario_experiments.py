...
def run_one(dlg_id: int, scenario: str, seed: int, run_ml: bool) -> list[dict]:
    label = f"DLG{dlg_id}"
    G = load_graph(label)

    try:
        splits = scenario_split(G, scenario=scenario, seed=seed)
    ...
    base = {
        ...
        "train_pos": len(pos_train), "test_pos": len(pos_test),
    ...
    for method in HEURISTICS:
        sc = scores_dict[method]
        t  = best_threshold_f1(y_true, sc) if len(np.unique(sc)) > 1 else 0.5
        m  = evaluate_split(splits, sc, threshold=t)
    ...
    for name, Cls in ML_MODELS.items():
        try:
            ...
            sc = clf.predict_proba(G_train, test_edges)
            m  = evaluate_split(splits, sc, threshold=0.5)
